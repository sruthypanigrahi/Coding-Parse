...
class TOCSearcher:
    ...
    def _build_index(self):
        ...
        toc_file = self._get_toc_file()
        if not toc_file:
            return
        
        self._load_entries_from_file(toc_file)
    ...
    def _get_toc_file(self) -> Path:
        """Get TOC file path if it exists"""
        toc_file = Path(TOC_OUTPUT_FILE)
        if not toc_file.exists():
            logger.warning(
                f"TOC file not found: {TOC_OUTPUT_FILE}"
            )
            return None
        return toc_file
    
    def _load_entries_from_file(self, toc_file: Path):
        """Load and index entries from TOC file"""
        self.entries = []
    ...
    def _process_line(self, line: str, line_num: int):
        """Process a single line from TOC file"""
        try:
    ...
    def _index_entry(self, entry: Dict):
        ...
        for word in words:
            self.word_index[word.lower()].add(entry_index)
